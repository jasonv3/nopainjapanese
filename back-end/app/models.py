...
class Post(PaginatedAPIMixin, db.Model):
    ...
    chinesebody = db.Column(db.Text)
    imgossdir = db.Column(db.Text)
    oggossdir = db.Column(db.Text)
    ...
    tagers = db.relationship('Tag', secondary=posts_tags, backref=db.backref('tags_post', lazy='dynamic'), lazy='dynamic')
    worders = db.relationship('Word', secondary=posts_words, backref=db.backref('words_post', lazy='dynamic'), lazy='dynamic')
    ...
    def to_dict(self):
        data = {
            ...
            'body': self.body,
            'chinesebody': self.chinesebody,
            'imgossdir': self.imgossdir,
            'oggossdir': self.oggossdir,
            'timestamp': self.timestamp,
            'tags': [
                {
                    'id': tag.id,
                    'tagname': tag.tagname,
                } for tag in self.tagers
            ],
            'words': [
                {
                    'id': word.id,
                    'wordname': word.wordname,
                } for word in self.worders
            ],
            'views': self.views,
    ...
    def from_dict(self, data):
        for field in ['title', 'summary', 'body','chinesebody','imgossdir','oggossdir', 'tags','timestamp', 'views']:
            if field in data:
                setattr(self, field, data[field])
    ...
    def is_taged_by(self, tag):
        '''判断文章是否已经有了这个tag'''
        return tag in self.tagers

    def taged_by(self, tag):
        '''加了tag'''
        if not self.is_taged_by(tag):
            self.tagers.append(tag)

    def untaged_by(self, tag):
        '''取消tag'''
        if self.is_taged_by(tag):
            self.tagers.remove(tag)

            
    def is_worded_by(self, word):
        '''判断文章是否已经有了这个word'''
        return word in self.worders

    def worded_by(self, word):
        '''加了word'''
        if not self.is_worded_by(word):
            self.worders.append(word)

    def unworded_by(self, word):
        '''取消tag'''
        if self.is_worded_by(word):
            self.worders.remove(word)
...
class Comment(PaginatedAPIMixin, db.Model):
    ...
    def from_dict(self, data):
        for field in ['body', 'timestamp', 'mark_read', 'disabled', 'author_id','oggossdir', 'post_id', 'parent_id']:
            if field in data:
                setattr(self, field, data[field])
...
class Tag(PaginatedAPIMixin, db.Model):  # 不需要分页
    __tablename__ = 'tags'
    id = db.Column(db.Integer, primary_key=True)
    ...
    def to_dict(self):
        data = {
            ...
            'timestamp': self.timestamp,
            '_links': {
                'self': url_for('api.get_tags'),
            }
        }
        return data

    def from_dict(self, data):
        for field in ['tagname','tagname']:
            if field in data:
                setattr(self, field, data[field])

class Word(PaginatedAPIMixin, db.Model):  # 不需要分页
    __tablename__ = 'words'
    id = db.Column(db.Integer, primary_key=True)
    wordname = db.Column(db.Text)
    timestamp = db.Column(db.DateTime, index=True, default=datetime.utcnow)
