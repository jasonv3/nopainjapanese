<template>
  <div class="container">
    <!-- Modal -->
    <div class="modal fade" id="dictModalCenter" tabindex="-1" role="dialog" aria-labelledby="dictModalCenterTitle" aria-hidden="true">
      <div class="modal-dialog modal-dialog-centered" role="document">
        <div class="modal-content">
          <div class="modal-header">
            <h5 class="modal-title" id="exampleModalLongTitle">{{dictWord}}</h5>
      ...
      <div class="col-lg-9">
        ...
        <article v-if="post" class="g-mb-30 g-pt-15 g-pb-15">
          <header class="g-mb-30">
            ...
            <div class="mb-3">
              <template v-for="(tag, index) in tags">
                <router-link :to="tag.url" v-bind:key="index">
                  <span class="badge badge-pill badge-info mr-1">{{tag.text}}</span>
                </router-link>
              </template>
            </div>
            <ul class="list-inline d-sm-flex g-color-gray-dark-v4 mb-0">
              <li v-if="post.author && (post.author.id == sharedState.user_id || sharedState.user_perms.includes('admin'))" class="list-inline-item">
                ...
                <router-link v-bind:to="{ path: '/edit/post/' + post.id }"><button class="btn btn-xs u-btn-outline-purple g-mr-5">编辑</button></router-link>
              </li>
              <li v-if="post.author && (post.author.id == sharedState.user_id || sharedState.user_perms.includes('admin'))" class="list-inline-item">
          ...
          <div id="postBody" class="g-font-size-16 g-line-height-1_8 g-mb-30">
            <p>
              <img v-show="post.imgossdir" :src="post.imgossdir" style="max-width: 320px;" />
            </p>
            <p>{{post.body}}</p>
            <hr/>
            <p>{{post.chinesebody}}</p>
            <hr/>
            <p>
              <audio v-show="post.oggossdir" :src="post.oggossdir" controls />
            </p>
            <div class="card">
              ...
              <div class="card-body">
                <template v-for="(word, index) in post.words">
                  <button v-bind:key="index" type="button" v-on:click="translate(word.id, word.wordname)" class="btn btn-outline-info mr-1 mb-1">{{word.wordname}}</button>
                </template>
              </div>
            </div>
          </div>
        ...
        <div id="comment-list-wrap" class="card border-0 g-mb-15">
          ...
          <form id="addCommentForm" v-if="sharedState.is_authenticated" @submit.prevent="onSubmitAddComment" @reset.prevent="onResetAddComment" class="g-mb-40">
            <div class="form-group">
              <textarea v-model="commentForm.body" class="form-control" id="commentFormBody" rows="3" placeholder=" 写下你的评论 ..."></textarea>
            ...
            <div v-for="(comment, index) in comments.items" v-bind:key="index">
              <div v-bind:id="'c' + comment.id" class="comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20">
                ...
                <div class="media-body">
                  ...
                  <div v-else>
                    ...
                    <p>{{comment.body}}</p>
                    <audio v-show="comment.oggossdir" :src="comment.oggossdir" controls />
                  </div>
              ...
              <div class="g-ml-40 comment-item media g-brd-around g-brd-gray-light-v4 g-pa-30 g-mb-20"
                ...
                <div class="media-body">
                  ...
                  <div v-else>
                    <p>{{child.body}}</p>
                    <audio v-show="child.oggossdir" :src="child.oggossdir" controls />
                  </div>
...
export default {
  ...
  methods: {
    ...
    async translate(id, wordname) {
      window.open(`/#/translation/${id}`);
      // this.dictWord = wordname;
      // this.translated = await this.getTransition(id);
      // $('#dictModalCenter').modal('show');
    },
    async getPost (id) {
    ...
    async onSubmitAddComment (e) {
      ...
      if (this.commentForm.parent_id) {
        // 说明是回复评论
        const at_who = `@${this.commentForm.author_name} `
        payload = {
  ...
  async created () {
    ...
    await this.getPostComments(post_id)
    // tooltip
